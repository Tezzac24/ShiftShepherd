...
- **Supabase** — production backend. Established live slices remain wired; Profile Editing V1 and Team Avatars V1 are live behind pushed `20260711041539_add_profile_editing_and_team_avatars.sql`. Profile edits are limited to the caller's full name through the one-argument `update_own_profile` — phone is read-only in the app, reserved for a future verified account flow, and the name-only corrective migration `20260711050344_restrict_profile_editing_to_name.sql` is local-only pending explicit push approval and live QA. Team photos are private, signed, and manageable only by team leaders/admins through `set_team_avatar_path`, with management UI on the dedicated Team Settings screen. Demo mode never calls either service.
...
### Backend Abstraction
Profile Editing V1 uses `profiles.ts` + the one-argument `update_own_profile(p_full_name)` for only `full_name` — phone is never sent (read-only in the app, reserved for a future verified account flow). Team Avatars V1 uses `teamAvatars.ts`, private `team-avatars` paths `teams/<teamId>/<file>`, signed URLs, and manager-only `set_team_avatar_path`; avatar management UI lives on the dedicated Team Settings screen (`app/teams/[teamId]/settings.tsx`), reached via a subtle header action visible only to leaders/admins. Neither path grants broad table UPDATE or anon access; demo ids are rejected.
