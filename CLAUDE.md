...
- **Supabase** — production backend. Profile Editing V1 and Team Avatars V1 are live behind pushed `20260711041539_add_profile_editing_and_team_avatars.sql` plus pushed/verified corrective `20260711050344_restrict_profile_editing_to_name.sql`; phone remains read-only and team photos stay private/manager-controlled. Organisation Directory + Team Membership Management V1 is implemented behind local-only `20260711063412_add_team_membership_management.sql`: dedicated Team Settings routes let live leaders/admins browse linked same-organisation profiles, add default members, and confirm removal of ordinary non-self members; leader removal remains blocked because role editing is out of scope. Demo mode stays read-only.
...
### Backend Abstraction
Profile Editing V1 uses `profiles.ts` + the one-argument `update_own_profile(p_full_name)` for only `full_name` — phone is never sent. Team Avatars V1 uses `teamAvatars.ts`, private `team-avatars` paths, signed URLs, and manager-only `set_team_avatar_path`. Team Membership Management V1 uses `teamMemberships.ts` plus authenticated-only `add_team_member(p_team_id, p_profile_id)` / `remove_team_member(p_team_id, p_profile_id)`: caller and organisation authority are server-derived, candidates must be Auth-linked profiles in the team's organisation, adds always use `member`, duplicate adds are idempotent, and leader/self removal is blocked. `team_memberships` remains SELECT-only to app roles; no direct write or anon grant is added. AppData patches successful rows immediately and quietly refreshes the directory. Demo/mock ids are rejected.
...
The Supabase MCP server is configured against the **dev** project. Remote history is aligned through pushed/verified `20260711050344_restrict_profile_editing_to_name.sql`; `20260711063412_add_team_membership_management.sql` is the only local-only migration. `send-chat-message-push` remains ACTIVE with JWT verification and physical-iPhone delivery QA pending. Never push migrations/deploy functions without explicit approval.
