...
- **Supabase** — production backend. Established profile/team/membership/shared-data slices are live. Chat unread cursor/RPC state is live behind applied immutable `20260711173139_add_team_chat_read_cursor.sql`; exactly one corrective migration, `20260711195143_migrate_chat_realtime_to_private_broadcast.sql`, is local-only and replaces only chat Postgres Changes with secure private Broadcast. Demo mode stays read-only and creates no live channel.
...
The Supabase **dev** project is aligned remotely through applied `20260711173139_add_team_chat_read_cursor.sql`, which is immutable. Only `20260711195143_migrate_chat_realtime_to_private_broadcast.sql` is local-only. Its controlled push must precede/ship with the Broadcast client and be followed by private-topic, two-user, multi-device, membership, token-refresh, and reconnect QA. `send-chat-message-push` remains ACTIVE and physical-iPhone delivery QA remains pending; push is independent of unread state. Never push migrations/deploy functions without explicit approval.
