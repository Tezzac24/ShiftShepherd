...
- **Supabase** — production backend. Established profile/team/membership/shared-data slices and secure private chat Broadcast are live; remote history is aligned through deployed/QA-complete `20260711195143`. The local-only `20260712001940_add_invite_onboarding_identity_foundation.sql` adds account-global identity, active multi-org profiles, open signup/no-org/create-org, and app-owned invitations. Its Edge Function and remote Auth/email configuration remain undeployed/unchanged. Demo mode stays isolated.
...
The Supabase dev project is aligned remotely through deployed and manually QA-complete `20260711195143_migrate_chat_realtime_to_private_broadcast.sql`. Only `20260712001940_add_invite_onboarding_identity_foundation.sql` is local-only. `manage-organisation-invitations` and the active-profile compatibility revision of `send-chat-message-push` are undeployed. Controlled rollout order is migration verification, invitation secrets/redirects, both function revisions, then disposable-account invitation/multi-org QA. Physical-iPhone push QA remains separate. Never push migrations, deploy functions, change remote Auth/email settings, or send live invitations without explicit approval.
