...
- **Supabase** — production backend. Established profile/team/membership/shared-data slices, secure private chat Broadcast, and the multi-organisation identity/onboarding foundation are deployed; remote history is aligned through `20260712001940`. `manage-organisation-invitations` v1 and active-profile-compatible `send-chat-message-push` v5 are ACTIVE, with runtime invitation configuration present. Live invitation/no-org device QA remains deferred. Demo mode stays isolated.
...
Organisation Membership & Role Management V1 is implemented locally in `c54da51` with access-loss invalidation hardened in `7628569`, behind `20260712103321_add_organisation_membership_role_management.sql`. Stable profiles gain explicit active/removed access; admin removal and caller leave revoke current roles/team memberships/profile push tokens while retaining history/global identity/other organisations; active-profile transitions are atomic; every removal invalidates and clears the affected account scope; all last-admin mutations share an organisation-row lock; removed-profile re-invitation restores only `general_member`. Profile exposes admin **Organisation members** and user-owned **Leave organisation** flows. The Supabase dev project remains aligned remotely through deployed `20260712001940`; the new migration is the only local-only mismatch. Its SQL has static contract coverage but no Docker runtime execution, deployment, or manual QA. `manage-organisation-invitations` v1 and active-profile-compatible `send-chat-message-push` v5 remain deployed/ACTIVE with invitation configuration present. Genuine no-organisation device QA, first real invitation delivery/acceptance, disposable-account invitation/multi-org QA, and physical-iPhone push QA remain pending. Production invitation delivery is not approved and custom-scheme links are not production-ready. Never push migrations, deploy functions, change remote Auth/email settings, or send live invitations without explicit approval.
