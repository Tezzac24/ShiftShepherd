...
> **Current dev-project state:** remote migration history is aligned through deployed `20260715004513_add_team_creation_editing_and_archive.sql` across 32 migrations; Push Token Lifecycle V1's database contract is live-verified and Team Creation & Editing V1 is deployed and hosted-contract verified, with no local-only, remote-only, or repair entry. Local typecheck, lint, 515/59 tests, migration validation, Android/iOS/web Expo export, and diff checks pass. `manage-organisation-invitations` v1 and active-profile-compatible `send-chat-message-push` v5 are deployed and ACTIVE; invitation runtime secrets and allowed redirects are configured, and non-mutating function smoke tests passed. The membership SQL was applied on 2026-07-12 and passed hosted schema/backfill/RLS/ACL/publication/data-preservation verification, but **no live membership, role, removal, leave, or re-invitation mutation has been performed**. Team lifecycle SQL passed hosted PostgreSQL compilation, RPC/ACL inspection, authenticated lifecycle probes, archive/restore, history preservation, RLS isolation, and database invariants (all mutations rolled back); its manual app QA has not run. Disposable-account membership/role/team lifecycle QA, genuine no-organisation device QA, first real invitation delivery/acceptance, disposable-account invitation/multi-org QA, and physical-iPhone push QA remain pending; real-user rollout stays blocked. **Do not rename or edit applied migrations.**
