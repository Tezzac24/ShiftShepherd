...
> **Current dev-project state:** remote migration history is tracked and aligned — `001`–`006`, the events/rota/songs/chat/notification-preferences grants migrations, the Auth/profile auto-link migration, the chat realtime publication migration, the chat read-states migration (`20260710031212`), and the profile avatar storage migration (`20260710105140`, private profile-avatars bucket + storage policies + avatar RPC) are all pushed — local and remote history are fully aligned. Migrations `001`–`002` were originally applied by hand and back-filled into history; later applied migrations used `supabase db push`. **Do not rename old or timestamped migrations.** See `docs/supabase-migration-alignment-checkpoint.md`.
...
│   └── 20260710105140_add_profile_avatar_storage.sql # private profile-avatars bucket, storage policies, avatar RPC
├── seed/
...
3. Apply migrations:
   - **Hosted dev:** remote history is aligned through `20260710105140`. New changes go through `supabase migration new <name>` (leave the generated filename unchanged) and an explicitly approved `supabase db push`. **Do not rename `001`–`006` or timestamped migrations.**
