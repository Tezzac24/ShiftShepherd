...
> **Current dev-project state:** remote migration history is aligned through deployed `20260712103321_add_organisation_membership_role_management.sql` across all 30 migrations, with no local-only migration, no remote-only migration, and no repair entry. Local typecheck, lint, 359/48 tests, migration validation, Android/iOS/web Expo export, and diff checks pass. `manage-organisation-invitations` v1 and active-profile-compatible `send-chat-message-push` v5 are deployed and ACTIVE (bundle hashes unchanged by the membership deployment); invitation runtime secrets and allowed redirects are configured, and non-mutating function smoke tests passed. The membership SQL was applied on 2026-07-12 and passed hosted schema/backfill/RLS/ACL/publication/data-preservation verification, but **no live membership, role, removal, leave, or re-invitation mutation has been performed**. Disposable-account membership/role QA, genuine no-organisation device QA, first real invitation delivery/acceptance, disposable-account invitation/multi-org QA, and physical-iPhone push QA remain pending; real-user rollout stays blocked. **Do not rename or edit applied migrations.**
...
Direct authenticated profile/role writes remain revoked and the old permissive role-write policy is dropped. Every new definer function has `search_path = ''`, fully qualified relations, server-derived caller/tenant authority, and explicit ACLs. The deterministic suite passes 359 tests across 48 suites.
...
The pre-membership deterministic regression suite passed 296 tests across 43 suites; the current implementation passes 359 tests across 48 suites (the deployed Broadcast baseline was 211/31). Edge/Deno runtime and provider delivery are outside Jest; non-mutating identity-function smoke tests passed, and the membership SQL is now applied and hosted-verified, while member/role manual QA and first real email delivery/acceptance remain manual gates.
...
3. Apply migrations:
   - **Hosted dev:** remote history is aligned through `20260712103321_add_organisation_membership_role_management.sql`, with no local-only or remote-only migration. New changes go through `supabase migration new <name>` (leave the generated filename unchanged) followed by an explicitly approved `supabase db push`. **Do not rename `001`–`006` or timestamped migrations.** `npm run check:migrations` is an offline filename guard; local/remote alignment still requires `npx supabase migration list`.
