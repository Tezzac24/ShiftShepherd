...
> **Current dev-project state:** remote migration history is tracked and aligned — `001`–`006`, the events/rota/songs/chat/notification-preferences grants migrations, the Auth/profile auto-link migration, the chat realtime publication migration, and the chat read-states migration (`20260710031212`) are all pushed. One migration is **local-only pending an explicitly approved push**: `20260710105140_add_profile_avatar_storage.sql` (private profile-avatars bucket + storage policies + avatar RPC — live photo uploads show a friendly setup message until it lands). Migrations `001`–`002` were originally applied by hand and back-filled into history; later applied migrations used `supabase db push`. **Do not rename old or timestamped migrations.** See `docs/supabase-migration-alignment-checkpoint.md`.
...
│   └── 20260710105140_add_profile_avatar_storage.sql # private profile-avatars bucket, storage policies, avatar RPC (local-only until pushed)
├── seed/
...
3. Apply migrations:
   - **Hosted dev:** remote history is aligned through `20260710031212`; `20260710105140_add_profile_avatar_storage.sql` is the one pending local-only migration. New changes go through `supabase migration new <name>` (leave the generated filename unchanged) and an explicitly approved `supabase db push`. **Do not rename `001`–`006` or timestamped migrations.**
