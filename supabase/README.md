...
> **Current dev-project state:** remote migration history is aligned through deployed `20260714200205_add_push_token_revocation.sql` across 31 migrations; its Push Token Lifecycle V1 database contract is live-verified. `20260715004513_add_team_creation_editing_and_archive.sql` is the sole newer local-only migration, with no remote-only migration and no repair entry. Local typecheck, lint, 515/59 tests, migration validation, Android/iOS/web Expo export, and diff checks pass. `manage-organisation-invitations` v1 and active-profile-compatible `send-chat-message-push` v5 are deployed and ACTIVE; invitation runtime secrets and allowed redirects are configured, and non-mutating function smoke tests passed. The membership SQL was applied on 2026-07-12 and passed hosted schema/backfill/RLS/ACL/publication/data-preservation verification, but **no live membership, role, removal, leave, or re-invitation mutation has been performed**. Team lifecycle SQL has static contract coverage only because local PostgreSQL is unavailable; its manual app QA has not run. Disposable-account membership/role/team lifecycle QA, genuine no-organisation device QA, first real invitation delivery/acceptance, disposable-account invitation/multi-org QA, and physical-iPhone push QA remain pending; real-user rollout stays blocked. **Do not rename or edit applied migrations.**
...
│   └── 20260715004513_add_team_creation_editing_and_archive.sql # local only: church-admin team lifecycle
├── functions/
...
Direct authenticated profile/role writes remain revoked and the old permissive role-write policy is dropped. Every new definer function has `search_path = ''`, fully qualified relations, server-derived caller/tenant authority, and explicit ACLs. The current deterministic suite passes 515 tests across 59 suites.
...
### Local-only team creation and archive lifecycle

`20260715004513_add_team_creation_editing_and_archive.sql` implements Team Creation & Editing V1 locally and is intentionally undeployed. It adds nullable `teams.archived_at` and `teams.archived_by`, with `archived_by` referencing `profiles(id) ON DELETE SET NULL`, plus partial organisation-scoped indexes for active and archived directory reads.
...
3. Apply migrations:
   - **Hosted dev:** remote history is aligned through `20260714200205_add_push_token_revocation.sql`; `20260715004513_add_team_creation_editing_and_archive.sql` is the sole local-only migration and must remain undeployed until a separately approved controlled deployment task. New changes go through `supabase migration new <name>` (leave the generated filename unchanged) followed by an explicitly approved `supabase db push`. **Do not rename `001`–`006` or timestamped migrations.** `npm run check:migrations` is an offline filename guard; local/remote alignment still requires `npx supabase migration list`.
