...
│   └── 20260917110331_add_announcement_push_delivery.sql # LOCAL-ONLY (pending deployment): announcement event type + service_role read grants
├── functions/
│   ├── send-chat-message-push/  # deployed v5 (chat); local kind-aware revision adds announcements (pending deployment); dispatch.ts holds the pure rules
...
The pre-membership deterministic regression suite passed 296 tests across 43 suites; the current local implementation passes 608 tests across 65 suites (the deployed Broadcast baseline was 211/31). Edge/Deno runtime and provider delivery are outside Jest (the pure invitation email template/expiry module is unit-tested); non-mutating identity-function smoke tests passed, and the membership SQL is applied and hosted-verified, while member/role/team lifecycle manual QA and first real email delivery/acceptance remain manual gates.
...
## Push delivery: chat messages (deployed) and announcements (local, pending deployment)
