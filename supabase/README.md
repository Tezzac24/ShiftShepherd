...
3. Apply migrations:
   - **Hosted dev (already aligned):** remote migration history tracks `001`–`006` plus the pushed events, rota, songs, chat, and notification-preferences grants migrations; use `supabase db push` for future changes after approval. **Do not rename `001`–`006`** — the remote history records those exact version strings, so renaming would desync history and re-apply migrations. Create new migrations with `supabase migration new <name>` and leave the generated timestamped filename unchanged.
