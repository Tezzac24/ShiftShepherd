...
`manage-organisation-invitations` v1 is deployed and ACTIVE; the local source additionally renders the invitation expiry as an explicit UTC date and time in `invitationEmail.ts` (identical to the app's `src/lib/invitations/expiry.ts`) and is pending deployment. Public preview is protected by the high-entropy app token and reveals only bounded invitation context; send/resend/revoke/accept manually validate the user JWT, and the service-only database operations repeat tenant/admin/identity checks. Raw tokens exist only in request memory and the email link. The isolated Resend adapter uses runtime-only `RESEND_API_KEY`, `INVITATION_FROM_EMAIL`, and `INVITATION_APP_BASE_URL`; the allowed `/invite/accept` redirect and sender are configured. No secrets belong in Expo.
