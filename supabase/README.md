...
> **Current dev-project state:** remote migration history is tracked and aligned through the profile avatar storage migration (`20260710105140`, private `profile-avatars` bucket + policies + avatar RPC). `20260710114621_add_announcement_image_storage.sql` is intentionally local-only pending an explicitly approved `supabase db push`; it adds a private `announcement-images` bucket for one optional image per announcement, not chat attachments or galleries. Migrations `001`–`002` were originally applied by hand and back-filled into history; later applied migrations used `supabase db push`. **Do not rename old or timestamped migrations.** See `docs/supabase-migration-alignment-checkpoint.md`.
...
│   └── 20260710114621_add_announcement_image_storage.sql # local-only: private announcement-images bucket + policies
├── seed/
...
3. Apply migrations:
   - **Hosted dev:** remote history is aligned through `20260710105140`; `20260710114621` is the one local-only migration awaiting an explicitly approved `supabase db push`. New changes go through `supabase migration new <name>` (leave the generated filename unchanged). **Do not rename `001`–`006` or timestamped migrations.**
