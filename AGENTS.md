...
The current project state is a functional scaffold with locally persisted mock data and optional Supabase slices. Profile Editing V1, Team Avatars V1, Organisation Directory + Team Membership Management V1, Membership Governance + Leave Team + Shared Live Data Freshness V1, server-authoritative unread, and the private chat Broadcast cutover are live and manually QA-complete. Remote migration history is aligned through deployed `20260711195143_migrate_chat_realtime_to_private_broadcast.sql`; chat no longer uses Postgres Changes, while the 13 non-chat shared-data tables still do. The local-only next slice adds open signup/no-organisation/create-organisation flows, app-owned invitations, account-global identity, and secure active multi-organisation profiles through `20260712001940_add_invite_onboarding_identity_foundation.sql` plus undeployed `manage-organisation-invitations`. Remote Auth/email configuration is unchanged and live invitation QA is pending. Physical-iPhone push delivery QA remains separate.
...
The deployed Broadcast baseline was 211 tests across 31 suites. The local onboarding implementation passes 254 tests across 40 suites, adding deterministic migration-history/security, account-context, no-org/create-org, pending-token, invitation admin/acceptance, display-name, and multi-org push-compatibility coverage.
