...
The current project state is a functional scaffold with locally persisted mock data and optional linked-user Supabase slices. Profile Editing V1 (self-owned full name only — phone is read-only, reserved for a future verified account flow) and Team Avatars V1 (private photos managed by team leaders/admins through a dedicated Team Settings screen) are live behind pushed `20260711041539_add_profile_editing_and_team_avatars.sql`; the corrective `20260711050344_restrict_profile_editing_to_name.sql` (name-only `update_own_profile`) is local-only and requires an explicitly approved push and live QA. Demo mode remains local and never calls these services. The established announcement/chat image, push-token, and chat-push state remains unchanged; physical-iPhone push delivery QA is still pending.
...
- Supabase Auth, Postgres, RLS, Realtime (team chat only), private Storage (profile/team avatars, announcement images, chat images; team avatars pending local migration push/QA), and the deployed `send-chat-message-push` Edge Function wired
...
V1 is a functional scaffold with mocked, locally persisted data plus optional Supabase email/password Auth and the documented live slices. Storage is limited to profile avatars, team avatars, one announcement image, and one chat image per message; team avatars remain local-only pending migration push/QA, while the other image slices are live and QA'd (no arbitrary files, galleries, or message edit/delete). Realtime stays limited to the open team chat. Push Token Registration V1 and chat-message delivery remain unchanged, with physical-iPhone delivery QA pending. Demo/local mode intentionally remains supported.
