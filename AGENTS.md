...
The current project state is a functional scaffold with locally persisted mock data and optional linked-user Supabase slices. Profile Editing V1, Team Avatars V1, and Organisation Directory + Team Membership Management V1 are live; `20260711063412_add_team_membership_management.sql` is pushed/verified and its original add/remove flow passed manual QA. Membership Governance + Leave Team + AppData Consistency + Shared Live Data Freshness V1 is implemented behind two local-only migrations: `20260711154126_refine_team_membership_governance.sql` and `20260711154134_enable_shared_live_data_realtime.sql`. It separates organisation authority from team role, protects the final team admin under a team-row lock, gives every member a dedicated Leave Team flow, keeps current-user memberships canonical across AppData/session selectors, and uses one shared Realtime invalidation channel plus AppState foreground catch-up. Demo mode remains read-only and creates no shared-data channel. Physical-iPhone push delivery QA is still pending.
...
The current regression baseline is 150 tests across 24 suites, including membership/Leave Team RPC payload and error contracts, canonical membership consistency, team-role permission gates, removal/leave confirmations and pending states, safe access loss, Realtime table-domain mapping/channel lifecycle/coalescing, AppState catch-up, and demo/session isolation.
...
- Supabase Auth, Postgres, RLS, shared-data Realtime invalidation plus separate open-team-chat Realtime, private Storage (profile/team avatars, announcement images, chat images), and the deployed `send-chat-message-push` Edge Function wired
