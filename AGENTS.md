...
The current project state is a functional scaffold with locally persisted mock data and optional linked-user Supabase slices. Profile Editing V1 (self-owned full name only — phone is read-only) and Team Avatars V1 are live behind pushed `20260711041539_add_profile_editing_and_team_avatars.sql` plus pushed/verified corrective `20260711050344_restrict_profile_editing_to_name.sql`. Organisation Directory + Team Membership Management V1 is implemented in app code behind local-only `20260711063412_add_team_membership_management.sql`: live team leaders/admins can browse linked same-organisation profiles, add ordinary members, and remove ordinary non-self members from dedicated Team Settings routes; leader removal stays blocked because role editing is out of scope. Demo mode remains read-only and never calls these services. Physical-iPhone push delivery QA is still pending.
...
The current regression baseline is 108 tests across 17 suites, including membership RPC payload/error contracts, eligible-profile selectors, permission gates, removal confirmation, pending/failure states, rendered add/remove refreshes, and demo isolation.
...
- Supabase Auth, Postgres, RLS, Realtime (team chat only), private Storage (profile/team avatars, announcement images, chat images), and the deployed `send-chat-message-push` Edge Function wired
...
Supabase integration stays isolated in `src/lib/supabase/`. **Profile editing** uses `services/profiles.ts` and authenticated `update_own_profile(p_full_name)` for the caller's name only; phone stays read-only. **Team avatars** use `services/teamAvatars.ts`, private paths, signed URLs, and manager-only `set_team_avatar_path`. **Team memberships** use `services/teamMemberships.ts` plus authenticated-only `add_team_member(p_team_id, p_profile_id)` / `remove_team_member(p_team_id, p_profile_id)`: caller and organisation authority are derived server-side, targets must be linked profiles from the team's organisation, adds always use `member`, duplicate adds are idempotent, and leader/self removal is blocked. `team_memberships` retains SELECT-only app grants; there are no direct writes or anon access. AppData applies successful membership results immediately and then quietly refreshes the existing directory. Demo/mock ids are refused and management UI is read-only in demo mode. In addition to these slices, **chat images** use `services/chatAttachments.ts`: one JPEG/PNG/WebP under 5 MB, private `chat-attachments` paths, atomic message+metadata RPCs, signed URLs, and best-effort cleanup. **Push tokens** use `services/pushTokens.ts` and `register_push_token`; **chat push delivery** uses `services/pushDelivery.ts` and the deployed server-side function. Arbitrary files, chat edit/delete, receipts, and push beyond team chat remain deferred. DB↔app mapping stays centralized; live ids are real UUIDs end-to-end and demo mode uses pure mock ids.

The Supabase MCP server is configured against the dev project — use it to inspect the live schema/data. Remote migration history is aligned through pushed/verified `20260711050344_restrict_profile_editing_to_name.sql`; `20260711063412_add_team_membership_management.sql` is the only local-only migration and awaits an explicitly approved push/live QA. `send-chat-message-push` is deployed and ACTIVE with JWT verification. Physical iPhone development-build QA remains pending. Do not expand delivery beyond chat messages until that QA passes. Do not rename `001`–`006` or timestamped migrations; create future migrations with `supabase migration new <descriptive_name>` and keep the generated filename. Never run `supabase db push`, `supabase functions deploy`, or another remote write without explicit approval.
...
V1 is a functional scaffold with mocked, locally persisted data plus optional Supabase email/password Auth and the documented live slices. Storage is limited to profile avatars, team avatars, one announcement image, and one chat image per message. Realtime stays limited to the open team chat. Team membership management is limited to existing linked profiles, default-member adds, and confirmed ordinary-member removal; invites, account creation, and team/organisation-role editing remain deferred. Push Token Registration V1 and chat-message delivery remain unchanged, with physical-iPhone delivery QA pending. Demo/local mode intentionally remains supported.
