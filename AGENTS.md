...
The current project state is a functional scaffold with locally persisted mock data and optional linked-user Supabase slices. Profile Editing V1 (self-owned name/phone only) and Team Avatars V1 (private photos managed by team leaders/admins) are implemented behind local-only migration `20260711041539_add_profile_editing_and_team_avatars.sql`; they require an explicitly approved push and live QA. Demo mode remains local and never calls these services. The established announcement/chat image, push-token, and chat-push state remains unchanged; physical-iPhone push delivery QA is still pending.
