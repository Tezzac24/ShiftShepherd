...
The current project state is a functional scaffold with locally persisted mock data and optional linked-user Supabase slices. Profile Editing V1, Team Avatars V1, Organisation Directory + Team Membership Management V1, and Membership Governance + Leave Team + AppData Consistency + Shared Live Data Freshness V1 are live and passed manual QA. Chat unread state is server-authoritative behind applied, immutable `20260711173139_add_team_chat_read_cursor.sql`. One forward corrective migration, `20260711195143_migrate_chat_realtime_to_private_broadcast.sql`, is local-only: it replaces only chat message/read-state Postgres Changes with minimal private database Broadcast, exact team/profile topic authorization, dynamic session-owned channels, and exact-message RLS fetches. Non-chat shared data remains on Postgres Changes; demo mode opens no live channel. Physical-iPhone push delivery QA is still pending and independent of unread truth.
...
The current regression baseline is 211 tests across 31 suites, including membership/Leave Team and shared-data freshness coverage plus chat read-state RPC contracts, pure unread reducers, exact Broadcast topics/payloads, corrective-migration security guards, exact-message RLS fetches, private channel/auth ordering, token refresh, dynamic membership channels, reconnect health, stale-fetch suppression, account-switch teardown, demo isolation, and a repository guard against chat-specific Postgres Changes.
...
Supabase integration stays isolated in `src/lib/supabase/`. **Profile editing**, **team avatars**, and governed **team memberships** retain their existing narrow RPC/service boundaries. Non-chat **shared live freshness** remains Postgres Changes invalidation through `sharedRealtime.ts`; this chat correction does not convert announcements/events/rotas/songs/directory. **Chat unread state** remains server-authoritative through `get_team_chat_unread_summary()` and forward-only `mark_team_chat_read(p_team_id, p_message_id)`; direct read-state writes stay revoked. `chatBroadcast.ts` builds exact `team-chat:<teamId>` and `profile-chat-read:<profileId>` topics, sets Realtime auth before private subscription and on token refresh, reconciles one channel per canonical accessible team plus one own-profile channel, strictly validates minimal v1 events, and fetches one message by `(team_id,id)` through existing RLS. The database trigger payload contains no message text or attachment/profile data, and `realtime.messages` has SELECT-only exact-topic policies—no client Broadcast INSERT/send path. Membership loss removes channels; reconnect/foreground/read invalidations coalesce through the existing summary scheduler; account switch tears down old channels; demo opens none. Chat images, push tokens, and chat push delivery remain unchanged. Push is an alert, never unread truth. Visible receipts, typing/presence, message edit/delete, native app-icon badge sync, and push expansion remain deferred.

The Supabase dev project is aligned remotely through applied `20260711173139_add_team_chat_read_cursor.sql`; that migration is immutable. Only `20260711195143_migrate_chat_realtime_to_private_broadcast.sql` is local-only. Its controlled deployment must verify the two minimal private triggers, fail-closed `realtime.messages` SELECT policies, no INSERT policy, and removal of only `chat_messages`/`chat_read_states` from `supabase_realtime`; then run private-topic two-user/multi-device/membership/token-refresh QA. `send-chat-message-push` remains ACTIVE and unchanged. Never push migrations/deploy functions without explicit approval.
...
V1 is a functional scaffold with mocked, locally persisted data plus optional Supabase email/password Auth and the documented live slices. Storage is limited to profile avatars, team avatars, one announcement image, and one chat image per message. Shared-data Realtime is Postgres Changes invalidation-only and session-scoped; chat uses private team/profile Broadcast topics with a server-authoritative unread cursor/summary. Visible read receipts, delivered/typing/presence, message edit/delete, native app-icon badge sync, and push expansion remain out of scope. Team membership management remains limited to existing linked profiles and governed membership changes; invites and role/team editing remain deferred. Push registration/delivery is unchanged, with physical-iPhone QA pending. Demo/local mode intentionally remains supported.
