...
Every migration through the chat image attachment permission fix (`20260710162415_fix_chat_image_attachment_permissions.sql`) is pushed and verified — live unread badges, profile photo uploads, announcement images, and chat image attachments are all active and QA'd. Chat supports exactly one optional image per message (no arbitrary files, audio/video, galleries, camera capture, full-screen viewer, or message edit/delete). Push Token Registration V1 is implemented behind the local-only `20260710171200_add_push_token_registration.sql` migration and needs an explicitly approved push before live registration QA.
...
- Supabase migrations are applied to the dev project with tracked history through the chat image attachment permission fix (`20260710162415`; see `docs/supabase-migration-alignment-checkpoint.md`); `20260710171200_add_push_token_registration.sql` is local-only pending an explicitly approved push. **Auth + live sessions + announcements (including images) + events + the read-only people/teams directory + rotas/availability + choir songs/song selections + team chat (realtime for the open conversation, plus one optional image per message) + notification preferences + profile avatar storage + device push token registration** are wired to live Supabase. Real push delivery stays deferred — registering a device stores its Expo push token, nothing more.
