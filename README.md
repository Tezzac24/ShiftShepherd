...
- Remote Supabase migrations are aligned through deployed `20260917110331_add_announcement_push_delivery.sql` (35 remote migrations); the team lifecycle, team role, team creation idempotency (`20260917093926`), and announcement push delivery (`20260917110331`) migrations are deployed and hosted-contract verified. `manage-organisation-invitations` v2 (UTC invitation-expiry presentation, deployed and verified 2026-09-17) and the kind-aware, active-profile-compatible `send-chat-message-push` v6 are deployed. Announcement Push Delivery V1 (church-wide and team announcement push through the same function and ledger, honouring the existing announcement preferences) is deployed and hosted-contract verified. Disposable-account invitation/multi-org QA, team lifecycle and team role manual app QA, and physical-device Expo ticket/banner QA are pending.
...
Invitation runtime uses configured `RESEND_API_KEY`, `INVITATION_FROM_EMAIL`, and `INVITATION_APP_BASE_URL`, an allowed `/invite/accept` Auth/app redirect, and a verified sender. These remain runtime-only and out of Expo and committed files. The migration, `manage-organisation-invitations` v2 (UTC invitation-expiry presentation, deployed and verified 2026-09-17 with `verify_jwt` and the invitation secrets unchanged; `send-chat-message-push` stayed at v5), and the kind-aware, active-profile-compatible `send-chat-message-push` v6 are deployed; use disposable accounts for the still-pending QA before any real invitations.
