...
**Status:** functional scaffold / demo MVP with optional Supabase Auth and local persistence. Established live slices cover announcements, events, people/teams, rotas, choir songs, chat, and notification preferences. **Profile Editing V1** lets linked users change only their own full name — phone is read-only, reserved for a future verified account flow; **Team Avatars V1** lets team leaders/admins manage one private team photo; and **Team Membership Management V1** now provides local app code for browsing linked organisation profiles and adding/removing ordinary team members from dedicated Team Settings screens. Membership writes await the new local-only migration and live QA. Demo mode remains read-only for these management services. Chat Message Push Delivery V1 remains deployed with physical-device delivery QA pending.
...
The Profile Editing V1 + Team Avatars V1 migration `20260711041539_add_profile_editing_and_team_avatars.sql` and corrective name-only migration `20260711050344_restrict_profile_editing_to_name.sql` are **pushed and verified**. Exactly one migration is local-only: `20260711063412_add_team_membership_management.sql`, which adds authenticated `add_team_member(uuid, uuid)` and `remove_team_member(uuid, uuid)` RPCs without broad membership table writes. Remote history is aligned through `20260711050344`.
...
- Team Membership Management V1 adds a **Manage members** row in Team Settings, a calm current-member list with protected leader/self rows, a searchable same-organisation linked-profile picker, confirmation before removal, immediate local refresh after successful RPCs, and friendly loading/empty/error states. Ordinary members see no management entry; demo mode remains read-only.
- Remote Supabase migrations are applied through `20260711050344`; `20260711063412_add_team_membership_management.sql` is local-only and awaits explicit push/QA. The deployed chat function remains unchanged and physical-device Expo ticket/banner QA is pending.
...
**Tests** live in `src/**/__tests__/*.test.ts(x)` and are deterministic: Supabase, Expo Notifications/Constants/Device, and AsyncStorage are mocked, no test uses the network or real credentials, and no real push token is ever generated. The current suite has **108 tests across 17 suites**. Coverage includes push/chat/notification regressions, profile and team-avatar contracts, and membership RPC payload boundaries, safe error mapping, selector/search behavior, permission gates, confirmation, pending states, rendered refreshes, and demo isolation.
...
**Still manual:** after the membership migration is explicitly pushed, verify a leader/admin can add one eligible linked profile, see the list/count update and persist after reopening, then remove that ordinary member through confirmation; verify leaders/cross-organisation/unlinked profiles stay protected, ordinary members see no management entry, and both screens remain comfortable on a small iPhone. Profile name-only editing and Team Settings/avatar correction have already passed manual QA. Physical-iPhone push banner QA remains pending. The Edge Function remains outside Jest.
...
When you are signed in through **Supabase Auth with a linked profile**, the announcements and calendar/events screens read and write the live `announcements` and `events` tables, the Teams/Messages/Home/Profile screens show your live teams, members, and organisation, and authorised leaders/admins can manage ordinary memberships from Team Settings once `20260711063412` is pushed. The rota screens read and write the live `rota_entries`, `rota_assignments`, and `availability_responses` tables, choir song screens read and write live `songs`, `song_links`, and `choir_rota_song_selections` rows, team chat reads and writes the live `chat_messages` table, and notification settings read/write `notification_preferences`. All of it goes through `src/lib/supabase/services/`; in demo mode everything keeps using local data and membership management stays read-only. Worth knowing:
...
- **Membership writes stay narrow.** `add_team_member` and `remove_team_member` derive the caller from `auth.uid()`, require `can_manage_team`, keep targets in the same organisation, add only Auth-linked profiles with the default `member` role, and block leader/self removal. The app role retains SELECT-only access to `team_memberships`; there is no anon access, invite/signup flow, role editor, or membership notification.
