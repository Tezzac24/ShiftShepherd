...
Remote history is aligned through applied `20260711173139_add_team_chat_read_cursor.sql`; its server-authoritative cursor, rollout backfill, RPCs, hardened grants, and temporary `chat_read_states` Postgres Changes publication are live and immutable. Exactly one migration is local-only: `20260711195143_migrate_chat_realtime_to_private_broadcast.sql`. It adds minimal private database Broadcast triggers and exact-topic `realtime.messages` SELECT policies, then removes only `chat_messages` and `chat_read_states` from `supabase_realtime`. The app-side Broadcast cutover must ship with or after a controlled push of that corrective migration, followed by two-user/multi-device QA.
...
- Remote Supabase migrations are applied through `20260711173139`; only `20260711195143_migrate_chat_realtime_to_private_broadcast.sql` is local-only. The deployed chat push function remains unchanged and physical-device Expo ticket/banner QA is pending.
...
**Tests** live in `src/**/__tests__/*.test.ts(x)` and are deterministic: Supabase, Expo Notifications/Constants/Device, and AsyncStorage are mocked, no test uses the network or real credentials, and no real push token is ever generated. The current suite has **211 tests across 31 suites** (up from 184/29). Coverage includes membership governance/Leave Team, canonical membership and shared-data freshness, plus chat read-state RPC contracts, unread reducers, exact Broadcast topics/payloads, migration security guards, exact-message RLS fetches, private channel/auth ordering, token refresh, dynamic membership channels, reconnect health, stale-fetch suppression, account-switch teardown, demo isolation, and the repository guard against chat-specific Postgres Changes.
...
**Still manual:** after the private-Broadcast corrective migration is explicitly pushed, use two live users/devices to verify private team/profile topic delivery, unread/read independence, church-admin access, membership removal/rejoin, token refresh, reconnect/foreground recovery, account switching, and multi-device read reconciliation. Physical-iPhone push banner QA remains separately pending; push is unrelated to unread truth. The Edge Function remains outside Jest.
