...
Local verification passes with 806 tests across 73 suites, typecheck, lint, migration filename validation, Android/iOS/web Expo export, and diff checks. Hosted migration history is aligned through `20260917124856_add_rota_push_delivery.sql` across 36 remote migrations, with no local-only, remote-only, or repair entry.
...
- Remote Supabase migrations are aligned through deployed `20260917124856_add_rota_push_delivery.sql` (36 remote migrations); the team lifecycle, team role, team creation idempotency (`20260917093926`), announcement push delivery (`20260917110331`), and rota push delivery (`20260917124856`) migrations are deployed and hosted-contract verified. `manage-organisation-invitations` v2 (UTC invitation-expiry presentation, deployed and verified 2026-09-17) and the kind-aware, active-profile-compatible `send-chat-message-push` v7 are deployed. Announcement Push Delivery V1 (church-wide and team announcement push through the same function and ledger, honouring the existing announcement preferences) and Rota Push Delivery V1 (rota assignment and rota change push, honouring the existing Rota updates preference) are deployed and hosted-contract verified. Disposable-account invitation/multi-org QA, team lifecycle and team role manual app QA, and physical-device Expo ticket/banner QA are pending.
...
Invitation runtime uses configured `RESEND_API_KEY`, `INVITATION_FROM_EMAIL`, and `INVITATION_APP_BASE_URL`. These remain runtime-only and out of Expo and committed files. The development project still sends from the Resend test sender (which delivers only to the Resend account owner) with `shiftshepherd://` links; `docs/production-email-readiness.md` lists the owner actions for a verified sending domain, an https link host, and Supabase Auth custom SMTP, Site URL, redirects, and templates. The local function source validates the three secrets before any invitation is issued (`INVITATION_EMAIL_NOT_CONFIGURED`), keeps a configured https base path, and reports provider failures as `EMAIL_DELIVERY_REJECTED` or `EMAIL_DELIVERY_UNAVAILABLE`; the admin history marks a saved invitation whose email was not sent, with Resend as the recovery. That source is pending deployment as `manage-organisation-invitations` v3. The migration, `manage-organisation-invitations` v2 (UTC invitation-expiry presentation, deployed and verified 2026-09-17 with `verify_jwt` and the invitation secrets unchanged; `send-chat-message-push` stayed at v5), and the kind-aware, active-profile-compatible `send-chat-message-push` v7 are deployed; use disposable accounts for the still-pending QA before any real invitations.
