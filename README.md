...
Invitation runtime uses configured `RESEND_API_KEY`, `INVITATION_FROM_EMAIL`, and `INVITATION_APP_BASE_URL`, an allowed `/invite/accept` Auth/app redirect, and a verified sender. These remain runtime-only and out of Expo and committed files. The migration, `manage-organisation-invitations` v2 (UTC invitation-expiry presentation, deployed and verified 2026-09-17 with `verify_jwt` and the invitation secrets unchanged; `send-chat-message-push` stayed at v5), and the kind-aware, active-profile-compatible `send-chat-message-push` v7 are deployed; use disposable accounts for the still-pending QA before any real invitations.
