...
Invitation runtime uses configured `RESEND_API_KEY`, `INVITATION_FROM_EMAIL`, and `INVITATION_APP_BASE_URL`, an allowed `/invite/accept` Auth/app redirect, and a verified sender. These remain runtime-only and out of Expo and committed files. The migration, `manage-organisation-invitations` v1, and active-profile-compatible `send-chat-message-push` v5 are deployed; the local `manage-organisation-invitations` source carries the UTC invitation-expiry presentation fix (app and email show `expires_at` as an explicit date and time in UTC) and is pending deployment; use disposable accounts for the still-pending QA before any real invitations.
