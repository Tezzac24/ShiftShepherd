...
Sign in with a linked Supabase account and open **Settings → Notifications**. Confirm that demo/local mode hides the Device notifications card and makes no push API or Supabase calls. In the iOS Simulator, confirm the card presents its friendly unsupported-simulator state without requesting permission or registering a token. Do not start Real Push Delivery V1 until a real Expo push token has been collected from a supported physical development build.
