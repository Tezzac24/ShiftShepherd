...
- Remote Supabase migrations are aligned through deployed `20260715004513_add_team_creation_editing_and_archive.sql` (32 remote migrations); the team lifecycle migration is deployed and hosted-contract verified, and local-only `20260719110500_add_team_role_management.sql` awaits its own review/deployment task. `manage-organisation-invitations` v1 and active-profile-compatible `send-chat-message-push` v5 are deployed. Disposable-account invitation/multi-org QA, team lifecycle and team role manual app QA, and physical-device Expo ticket/banner QA are pending.
