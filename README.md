...
Known accepted polish deferral: the card's "registered" state is session-only and doesn't persist after leaving and returning to the screen — re-registering succeeds, which is fine for now. Real Push Delivery V1 is not yet implemented; when it starts it must start narrowly with chat messages only.
