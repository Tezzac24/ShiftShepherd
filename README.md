...
**Status:** functional scaffold / demo MVP with optional Supabase Auth and local persistence. Established live slices cover announcements, events, people/teams, rotas, choir songs, chat, and notification preferences. **Profile Editing V1** lets linked users change only their own full name — phone is read-only; **Team Avatars V1** manages one private team photo; and the pushed/QA-complete **Team Membership Management V1** lets authorised leaders/admins browse linked organisation profiles and add/remove ordinary members. The new local-only Membership Governance + Leave Team and Shared Live Data Realtime migrations refine admin-removal rules, protect the final team admin transactionally, add a dedicated Leave Team flow, keep My Teams canonical, and add active-session Realtime invalidation plus foreground catch-up. Demo mode stays read-only and opens no shared-data channel. Chat Message Push Delivery V1 remains unchanged with physical-device delivery QA pending.
...
The Profile Editing V1 + Team Avatars V1 migrations and `20260711063412_add_team_membership_management.sql` are **pushed and verified**; Team Membership Management V1 passed manual QA and remote history was aligned through `20260711063412` before this pass. Exactly two migrations are now local-only: `20260711154126_refine_team_membership_governance.sql` and `20260711154134_enable_shared_live_data_realtime.sql`. They must not be treated as live until a controlled push and verification task.
...
- Remote Supabase migrations are applied through `20260711063412`; only `20260711154126` and `20260711154134` are local-only. The deployed chat function remains unchanged and physical-device Expo ticket/banner QA is pending.
...
**Tests** live in `src/**/__tests__/*.test.ts(x)` and are deterministic: Supabase, Expo Notifications/Constants/Device, and AsyncStorage are mocked, no test uses the network or real credentials, and no real push token is ever generated. The current suite has **150 tests across 24 suites** (up from 108/17). Coverage now includes membership governance and Leave Team RPC boundaries/errors, team-role permission presentation, canonical membership consistency, account-switch safeguards, Leave Team confirmations/pending/safe navigation/access loss, Realtime table-domain mapping, channel cleanup/reconnect, refresh coalescing/in-flight follow-up, and AppState foreground catch-up.
