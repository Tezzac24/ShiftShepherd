...
**Status:** functional scaffold / demo MVP with optional Supabase Auth and local persistence. **Announcements, events, the people/teams directory, rotas/availability, choir songs/song selections, and team chat are the live Supabase data slices**: when you sign in with Supabase Auth (and your auth user is linked to a profile), announcements, calendar events, your teams/members (read-only), team rotas with availability responses, choir songs/selections, and team chat messages load from the live database, protected by RLS — and your session itself (profile, role, memberships) is built from live rows. Notification preferences are still mocked/local, and the whole app — live slices included — still works fully offline in demo mode. The chat grants migration is local but not pushed yet, so live chat screens show a friendly load-error state until it is applied.
...
- Supabase migrations `001`–`006` plus the events, rota, and songs grants migrations (`20260709093129`, `20260709154733`, `20260709171613`) are applied to the dev project with tracked history (see `docs/supabase-migration-alignment-checkpoint.md`); **auth + live sessions + announcements + events + the read-only people/teams directory + rotas/availability + choir songs/song selections + team chat** are wired to live Supabase. The chat grants migration (`20260709205903_grant_authenticated_chat_api_privileges.sql`) exists locally but is **not pushed yet**; until it is applied, live chat screens show a friendly "couldn't load messages" state with retry. Notification preferences stay mocked/local for now.
...
- Live announcements, events, the teams directory, rotas, choir songs/selections, and chat messages are session data: they are not saved into the demo AsyncStorage snapshot, and **Reset Demo Data** does not touch the live database.
- Chat is text-only and has **no realtime yet**: live messages refresh when you open a chat, after you send, and via the visible "Check for new messages" bar. Unread badges are a demo-only simulation, so live mode shows none; attachments stay a "coming soon" placeholder in both modes.
...
Changes you make in the app (announcements, events, rotas, songs, messages, notification settings) are saved on-device with a versioned envelope. Corrupt or outdated saved data is discarded safely; the app falls back to the original mock examples. **Profile -> Reset Demo Data** restores everything to the original seed state. (In live mode, announcements, events, rotas, choir songs/selections, and chat messages come from Supabase instead and are unaffected by local persistence or the demo reset.)
