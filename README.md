...
**Status:** functional scaffold / demo MVP with optional Supabase Auth and local persistence. Established live slices cover announcements, events, people/teams, rotas, choir songs, chat, and notification preferences. **Profile Editing V1** lets linked users change only their own full name — phone is read-only; **Team Avatars V1** manages one private team photo; the pushed/QA-complete **Team Membership Management V1** lets authorised leaders/admins browse linked organisation profiles and add/remove ordinary members; and the pushed/QA-complete **Membership Governance + Leave Team + Shared Live Data Realtime V1** refines admin-removal rules, protects the final team admin transactionally, adds a dedicated Leave Team flow, keeps My Teams canonical, and adds active-session Realtime invalidation plus foreground catch-up. The new local-only **Chat Unread State & Session-Wide Messaging Freshness V1** makes unread state server-authoritative and multi-device consistent and moves chat freshness onto one session-scoped channel, so per-team badges and the Messages-tab badge update from anywhere in the app. Demo mode stays read-only and opens no shared-data or chat channel. Chat Message Push Delivery V1 remains unchanged with physical-device delivery QA pending.
...
Remote history is aligned through **pushed and verified** `20260711154134_enable_shared_live_data_realtime.sql`; `20260711154126_refine_team_membership_governance.sql` and `20260711154134` are pushed/verified and membership governance, Leave Team, canonical My Teams, shared Realtime, and foreground catch-up passed manual QA (the two multi-admin manual branches remain deferred until a safe multi-admin fixture exists). Exactly one migration is now local-only: `20260711173139_add_team_chat_read_cursor.sql` (the server-authoritative chat read cursor + unread summary + `chat_read_states` publication). It must not be treated as live until a controlled push and verification task followed by light two-user/multi-device QA.
...
- Remote Supabase migrations are applied through `20260711154134`; only `20260711173139_add_team_chat_read_cursor.sql` is local-only. The deployed chat function remains unchanged and physical-device Expo ticket/banner QA is pending.
...
**Tests** live in `src/**/__tests__/*.test.ts(x)` and are deterministic: Supabase, Expo Notifications/Constants/Device, and AsyncStorage are mocked, no test uses the network or real credentials, and no real push token is ever generated. The current suite has **184 tests across 29 suites** (up from 150/24). Coverage now includes membership governance and Leave Team RPC boundaries/errors, team-role permission presentation, canonical membership consistency, account-switch safeguards, Leave Team confirmations/pending/safe navigation/access loss, Realtime table-domain mapping, channel cleanup/reconnect, refresh coalescing/in-flight follow-up, AppState foreground catch-up, and — for chat unread — the read-state service RPC contracts (only team/message ids), pure unread reducers (own/duplicate/active-team/independence/prune), the session chat channel, the single-flight reconciliation scheduler, and the session messaging lifecycle (demo/logged-out/unlinked gating, reconnect/foreground/read-state reconciliation, account-switch teardown).
...
**Still manual:** after the two new migrations are explicitly pushed, use two live users/devices to verify ordinary church-admin membership removal, non-final/final team-admin rules, each Leave Team role path, immediate My Teams consistency, active Realtime updates, background→foreground catch-up, and small-iPhone comfort. The earlier add/remove membership slice already passed manual QA. Physical-iPhone push banner QA remains separately pending. The Edge Function remains outside Jest.
...
- **Shared freshness is invalidation + refetch.** The local-only publication migration adds `announcements`, `events`, rota tables, song tables, `organisations`, `profiles`, `teams`, `team_memberships`, and `organisation_roles` to `supabase_realtime`. Raw payloads only select a scoped loader; they never reconstruct joined state. Repeated events coalesce, reconnects catch up, logout/user switch removes channels/timers, and AppState foreground always refreshes shared domains. A removed user may not receive the membership DELETE under RLS, so foreground catch-up remains the authoritative correction.
...
- Chat freshness is **session-scoped, not screen-scoped**: one Realtime channel per linked profile (`chat-session:<profileId>`) streams every accessible new message plus your own read-state changes, so previews, per-team badges, and the Messages-tab badge update wherever you are in the app — you no longer have to open Messages first. Realtime is a fast delta path; the authoritative unread counts come from the server summary RPC, which the app reconciles (coalesced, single-flight) on session start, subscribe, reconnect, app foreground, membership change, and read-state events. **Unread state is server-authoritative and multi-device consistent** (`chat_read_states`, one row per person + team, a forward-only cursor derived from a real message): messages from other people after your read cursor count on the Messages tab, actually viewing a team chat advances the cursor (a read on one device clears the same team on another), your own messages never count for you, opening the Messages list alone marks nothing read, and nobody ever sees anyone else's read state — these are unread badges, not read receipts. Rollout initialises existing memberships to the latest message, so deployment starts with no historical unread flood. Chat Image Attachments V1 adds one optional JPEG/PNG/WebP image (under 5 MB) per live message in a private `chat-attachments` bucket, with image-only messages supported; its migrations (`20260710124206` plus the `20260710162415` permission fix) are pushed and QA'd, so live chat images are active. Demo chat stays local and text-only and opens no channel.
