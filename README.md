...
Local verification passes with 642 tests across 67 suites, typecheck, lint, migration filename validation, Android/iOS/web Expo export, and diff checks. Hosted migration history is aligned through `20260917110331_add_announcement_push_delivery.sql` across 35 remote migrations, with no local-only, remote-only, or repair entry.
...
- Notification settings save one row per person (`unique(user_id)` upsert; the row is only created on your first change). The screen also has a **Device notifications** card (live mode only): tapping **Enable device notifications** asks for OS permission (only then), fetches this device's Expo push token, and registers it through a narrow `register_push_token` RPC — re-registering just refreshes the row, and a shared phone follows whoever is signed in. The deployed push-delivery function safely skips recipients without a token, and its kind-aware v6 revision (deployed 2026-09-17) also delivers church-wide and team announcement pushes to the people who may read them, honouring the **Church announcements** and **Team announcements** toggles; no real Expo ticket or banner has yet been verified. A development build is needed for a reliable real-device token (Expo Go can't register); iOS Simulator token-registration UI was exercised, while Android and physical-iPhone QA are deferred.
