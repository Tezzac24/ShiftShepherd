...
Every migration through the announcement image storage migration (`20260710114621_add_announcement_image_storage.sql`) is pushed and verified — live unread badges, profile photo uploads, and announcement images are active and QA'd. Chat Image Attachments V1 is implemented behind the local-only `20260710124206_add_chat_image_attachments.sql` migration and needs an explicitly approved push before live QA.
