...
Local verification passes with 608 tests across 65 suites, typecheck, lint, migration filename validation, Android/iOS/web Expo export, and diff checks. Hosted migration history is aligned through `20260917093926_add_team_creation_idempotency.sql` across 34 remote migrations, with no local-only, remote-only, or repair entry.
...
- Remote Supabase migrations are aligned through deployed `20260719110500_add_team_role_management.sql` (33 remote migrations); the team lifecycle and team role migrations are both deployed and hosted-contract verified, and the team creation idempotency migration `20260917093926` is deployed and hosted-contract verified. `manage-organisation-invitations` v2 (UTC invitation-expiry presentation, deployed and verified 2026-09-17) and active-profile-compatible `send-chat-message-push` v5 are deployed. Announcement Push Delivery V1 (church-wide and team announcement push through the same function and ledger, honouring the existing announcement preferences) is implemented locally behind `20260917110331_add_announcement_push_delivery.sql` and is pending deployment. Disposable-account invitation/multi-org QA, team lifecycle and team role manual app QA, and physical-device Expo ticket/banner QA are pending.
...
Invitation runtime uses configured `RESEND_API_KEY`, `INVITATION_FROM_EMAIL`, and `INVITATION_APP_BASE_URL`, an allowed `/invite/accept` Auth/app redirect, and a verified sender. These remain runtime-only and out of Expo and committed files. The migration, `manage-organisation-invitations` v2 (UTC invitation-expiry presentation, deployed and verified 2026-09-17 with `verify_jwt` and the invitation secrets unchanged; `send-chat-message-push` stayed at v5), and active-profile-compatible `send-chat-message-push` v5 are deployed; use disposable accounts for the still-pending QA before any real invitations.
...
- Notification settings save one row per person (`unique(user_id)` upsert; the row is only created on your first change). The screen also has a **Device notifications** card (live mode only): tapping **Enable device notifications** asks for OS permission (only then), fetches this device's Expo push token, and registers it through a narrow `register_push_token` RPC — re-registering just refreshes the row, and a shared phone follows whoever is signed in. The deployed chat-delivery function safely skips recipients without a token, and its local kind-aware revision (pending deployment) also delivers church-wide and team announcement pushes to the people who may read them, honouring the **Church announcements** and **Team announcements** toggles; no real Expo ticket or banner has yet been verified. A development build is needed for a reliable real-device token (Expo Go can't register); iOS Simulator token-registration UI was exercised, while Android and physical-iPhone QA are deferred.
