...
**Current status on 2026-09-18: remote history is aligned through deployed `20260917223118_fix_chat_read_cursor_conflict_target.sql`. All 38 remote migrations are applied, with no local-only, remote-only, or repair entry.**
This document preserves the earlier pre-Broadcast and membership-start checkpoints as history, then records the deployed invitation/onboarding, membership, push-token lifecycle, Team Creation & Editing V1, Team Role Management V1, Team Creation Idempotency V1, Announcement Push Delivery V1, Rota Push Delivery V1, Invitation Acceptance Fix V1, and Chat Read Cursor Fix V1 slices.
...
- `20260917180127_fix_invitation_acceptance_role_conflict_target.sql` (**deployed 2026-09-17; hosted-contract verified; native invitation QA pending**): replaces `accept_organisation_invitation_internal(uuid, text, text)` with the definition deployed by `20260712103321`, changing one statement — the baseline role insert now uses `on conflict on constraint organisation_roles_organisation_id_user_id_key` instead of the unqualified `on conflict (organisation_id, user_id)`. PL/pgSQL exposes the function's `returns table` columns as variables, so that target was ambiguous with the `organisation_roles.organisation_id` column and PostgreSQL raised SQLSTATE 42702 on every first-time acceptance, rolling the whole call back; the Edge Function reported `INVITATION_REQUEST_FAILED` and no invitation could be accepted. The signature, result columns, `SECURITY DEFINER`, `search_path=''`, row locks and lock order, verified-email and status checks, removed-profile reactivation (baseline `general_member` only), and service-role-only EXECUTE are unchanged; no table, policy, trigger, index, other grant, or existing row is touched. Because acceptance always rolled back, no partially accepted row can exist; hosted verification should still confirm that no `organisation_invitations` row is `accepted` without a matching active profile. Created with `supabase migration new`; static contract tests verify all 36 earlier migration bytes unchanged and that the replacement differs from the deployed definition only in that one line. Applied 2026-09-17 from merge commit `ffeb224` after a dry run that listed only this migration, with no repair; remote history then aligned 37/37. Hosted verification the same day confirmed the deployed body is byte-identical to the migration (single overload, `SECURITY DEFINER`, `search_path=''`, owned by `postgres`, EXECUTE for `service_role` only with none for `authenticated`, `anon`, or `PUBLIC`, constraint-based conflict target present, no carriage returns) and that a static check of the deployed function reports zero errors. A transaction-scoped probe on the QA organisations, ending in `ROLLBACK`, proved the previously failing path now succeeds: a first-time acceptance linked the profile to the caller, granted `general_member` only, transitioned the active profile, marked the invitation accepted by that caller, left the other organisation untouched, and raised no 42702; an immediate replay returned the already-accepted result; a wrong-email caller was still refused with `INVITATION_EMAIL_MISMATCH` and the invitation stayed pending; and after promotion, a team membership, and removal, re-invitation reactivated the same profile with `general_member` only and no restored team membership. Post-rollback the database was unchanged: 11 profiles, 11 organisation roles, 8 user accounts, one pending invitation and none accepted, no profile without a role, zero residue, and identical Grace Community Church and QA fingerprints. Security advisors were unchanged (47 definer, 2 RLS-without-policy, 1 leaked-password) and performance advisors changed only in usage-driven unused-index statistics (20 to 18); no Edge Function was touched (`manage-organisation-invitations` v3, `send-chat-message-push` v7). No email was sent and no Auth setting changed.

- `20260917223118_fix_chat_read_cursor_conflict_target.sql` (**deployed 2026-09-18; hosted-contract verified; physical-device chat QA pending**): replaces `mark_team_chat_read(uuid, uuid)` with the definition deployed by `20260711173139`, changing one statement — the read-cursor upsert now uses `on conflict on constraint chat_read_states_user_id_team_id_key` instead of the unqualified `on conflict (user_id, team_id)`, which PL/pgSQL resolved against the `team_id` result column of the function and raised SQLSTATE 42702 on every call from 11 July 2026. Nothing else changes: the signature, the three result columns, the forward-only guard, the caller derived from `auth.uid()`, the `can_access_team` gate, the server-read cursor timestamp, `SECURITY DEFINER`, the pinned empty `search_path`, volatility, and authenticated-only EXECUTE are identical, and no table, column, policy, trigger, index, publication, or row is touched. The deployed body is byte-identical to the migration source after line-ending normalisation.
