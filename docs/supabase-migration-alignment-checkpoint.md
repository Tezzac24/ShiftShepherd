...
The remote dev schema and migration history are aligned through deployed `20260715004513` across 32 remote versions; the only local-only entry is the **undeployed** `20260719110500_add_team_role_management.sql` (Team Role Management V1), and there is no remote-only or repair entry. Push Token Lifecycle V1's database contract is live-verified and Team Creation & Editing V1 is deployed and hosted-contract verified. The Team Creation & Editing V1 deployment changed no Edge Function, Auth/email, secret, Realtime, invitation, or real-organisation data, and the local team role slice has made no remote change of any kind.
...
the chat tables remain outside it. The current local implementation (through Team Role Management V1) exports for Android, iOS, and web; its deterministic suite passes 565/61.
