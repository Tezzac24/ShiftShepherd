...
the chat tables remain outside it. The current implementation (through the local-only Team Creation Idempotency V1) exports for Android, iOS, and web; its deterministic suite passes 589/63.
...
589 tests across 63 suites. Commits `420e12f` and `bf6a538` repaired auth sign-out
