...
### 11. Push notifications — preferences ✅; token registration ✅ (iOS Simulator QA passed); chat delivery implemented locally (not live)
...
**Push Token Registration V1** shipped next (2026-07-10; migration pushed and DB/RPC-verified; iOS Simulator QA passed 2026-07-11). Registration only — nothing is delivered. What shipped:
...
**Chat Message Push Delivery V1** followed (implemented locally 2026-07-11; **not live** — Edge Function undeployed, migration local-only). Chat messages only; server-side only:
...
5. Going live requires (in order, each explicitly approved): `npx supabase db push` (applies `20260710234443`), `npx supabase functions deploy send-chat-message-push`, then manual QA: user A registers a token, user B sends in a shared team → A gets one push; the sender gets nothing; a user with chat notifications off gets nothing; demo mode sends nothing. Announcement/event/rota/availability delivery remains unimplemented.
...
**QA passed 2026-07-11 on a supported Simulator development build:** the live card appears, permission is requested only after the tap, registration succeeds, no raw token is shown, and a second signed-in user can take over the same simulator token. Known accepted polish deferral: the card's "registered" state is session-only and resets after leaving and returning; re-registering succeeds. With a token collected from a supported development build, Real Push Delivery V1 was unblocked and started narrowly with chat messages only — Chat Message Push Delivery V1 is implemented locally (step 11) and awaits its approved migration push + Edge Function deployment.
...
- **Notification delivery** — preferences persist, Push Token Registration V1 stores tokens (iOS Simulator QA passed), and Chat Message Push Delivery V1 is implemented locally — but nothing pushes until the `20260710234443` migration is pushed and the `send-chat-message-push` Edge Function is deployed (both explicitly approved). Announcements/events/rota/availability delivery stays unimplemented.
...
13. ✅ **Done (2026-07-10; iOS Simulator QA passed 2026-07-11):** **Push Token Registration V1** — Expo push tokens register from an explicit user action in notification settings, with pushed and verified `20260710171200_add_push_token_registration.sql`. iOS Simulator QA confirmed: the live card appears, permission is requested only after the tap, registration succeeds, no raw token is shown, and a second user can take over the same simulator token. Android and physical iPhone QA are deferred, and the card's session-only "registered" state (resets on leave/return; re-registering succeeds) is an accepted polish deferral. Real Push Delivery V1 started narrowly with chat messages only (item 14).
14. 🚧 **Implemented locally (2026-07-11), not live:** **Chat Message Push Delivery V1** — the `send-chat-message-push` Edge Function, the local-only `20260710234443_add_push_delivery_foundation.sql` delivery-ledger migration, and the best-effort `pushDelivery.ts` trigger on the live chat send path (see step 11). Next actions, each explicitly approved and in order: push the migration (Codex/Terra Low per the usual workflow), deploy the Edge Function, then manual two-user QA (recipient with token gets exactly one push; sender gets nothing; chat-notifications-off user gets nothing; demo mode sends nothing). Only after that QA passes should announcement/event/rota delivery even be planned.
