...
### 11. Push notifications — preferences ✅ (done); token registration app-done (migration pending); delivery deferred
...
5. EAS readiness shipped alongside: `eas.json` (development/preview/production profiles), `extra.eas.projectId` (`npx eas-cli init` while logged in as the project owner), stable `android.package` / `ios.bundleIdentifier`, and the `expo-notifications` config plugin. **Real-token QA needs a development build** — e.g. `npx eas-cli build --profile development --platform android` (platform credentials are only needed at build time, not for this code). In Expo Go the card shows the friendly needs-a-build copy instead.
...
- **Notification delivery** — preferences persist and Push Token Registration V1 can store this device's Expo push token (step 11; needs the migration push + a development build), but nothing pushes until a later Edge Function delivery pass.
...
13. ✅ **App done (2026-07-10), migration pending:** **Push Token Registration V1** — Expo push tokens register from an explicit user action in notification settings, with local-only `20260710171200_add_push_token_registration.sql`. Next action is explicit push approval, then manual QA on a development build (`npx eas-cli build --profile development --platform android`). Real push delivery remains a separate, later slice.
