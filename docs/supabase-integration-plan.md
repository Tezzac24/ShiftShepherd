...
### 11. Push notifications — preferences ✅; token registration DB/RPC ✅ (runtime QA pending); delivery deferred
...
**Push Token Registration V1** shipped next (2026-07-10; migration pushed and DB/RPC-verified; runtime QA pending). Registration only — nothing is delivered. What shipped:
...
- **Notification delivery** — preferences persist and Push Token Registration V1 can store this device's Expo push token (step 11; migration is pushed; real-token QA still needs a supported physical development build), but nothing pushes until a later Edge Function delivery pass.
...
13. ✅ **DB/RPC done (2026-07-10), runtime QA pending:** **Push Token Registration V1** — Expo push tokens register from an explicit user action in notification settings, with pushed and verified `20260710171200_add_push_token_registration.sql`. Next action is iOS Simulator QA on a MacBook (`npx eas-cli build --profile ios-simulator --platform ios`, then `npx expo start --dev-client`), which checks the simulator's friendly unsupported state. Do not start Real Push Delivery V1 until a real Expo token has been collected from a supported physical development build.
