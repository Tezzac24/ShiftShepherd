...
### 11. Push notifications — preferences ✅; token registration ✅ (iOS Simulator QA passed); delivery not yet implemented
...
6. Still deferred: real push delivery (Edge Functions/triggers/workers/cron), push receipts, unregister/"your devices" management, automatic background re-registration, and native FCM/APNs token storage. Delivery will later go through a Supabase Edge Function filtered by `notification_preferences`.
...
**QA passed 2026-07-11 on a supported Simulator development build:** the live card appears, permission is requested only after the tap, registration succeeds, no raw token is shown, and a second signed-in user can take over the same simulator token. Known accepted polish deferral: the card's "registered" state is session-only and resets after leaving and returning; re-registering succeeds. With a token collected from a supported development build, Real Push Delivery V1 is unblocked — it must start narrowly with chat messages only.
...
- **Notification delivery** — preferences persist and Push Token Registration V1 can store this device's Expo push token (step 11; migration pushed; iOS Simulator QA passed, Android/physical-iPhone QA deferred), but nothing pushes until a later Edge Function delivery pass.
